    ...
    for (u, d), g in raw_all.groupby(['user','day']):
        ...
        for pc, gp in g.sort_values('date').groupby('pc'):
            ...
            for _, row in gp.iterrows():
                ...
                if 'connect' in act:
                    connect_q.append(ts)
                elif 'disconnect' in act and connect_q:
...
def merge_all(data_dir, answers_path, chunk_bytes=1024**3):
    ...
    roll_cols = {}
    for col in feature_cols:
        roll_mu_prev, roll_sd_prev = per_user_rolling_stats(per_user, col, w)
        roll_sd_prev = roll_sd_prev.replace(0, np.nan)
        roll_cols[f'{col}_user_z_{w}d'] = ((per_user[col] - roll_mu_prev) / roll_sd_prev).fillna(0.0).astype('float32')

    per_user_out = pd.concat(
        [base_ids, pd.DataFrame(expand_cols, index=per_user.index), pd.DataFrame(roll_cols, index=per_user.index)],
        axis=1
    )
    ...
    # 7d rolling baselines (per day) only
    w = 7
    for col in feature_cols:
    ...
    for col in feature_cols:
        ...
        mu_r = row_level[f'{col}_global_rollmu_prev_7d']
        sd_r = row_level[f'{col}_global_rollsd_prev_7d'].replace(0, np.nan)
        glob_cols[f'{col}_global_z_7d'] = ((row_level[col] - mu_r) / sd_r).fillna(0.0).astype('float32')
...
if __name__ == '__main__':
    ...
    # Write CSVs
    out_dir = this_dir
    per_user_path = os.path.join(out_dir, 'per_user_features_timeaware_pruned1.csv')
    global_path   = os.path.join(out_dir, 'global_features_timeaware_pruned1.csv')
