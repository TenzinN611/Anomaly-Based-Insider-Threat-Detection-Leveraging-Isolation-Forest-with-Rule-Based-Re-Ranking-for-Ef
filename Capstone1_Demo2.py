# streamlit_anomaly_dashboard_updated.py
"""
...
st.set_page_config(layout="wide", page_title="Anomaly Detection (Dark Centered)")
...
def compute_budgets_table(ranked_df, budgets_counts, percent_budgets, total_tp):
    rows = []
    for k in budgets_counts:
        ...
        rows.append({'budget_type':'top_n','budget':k,'precision_pct':precision_pct,'coverage_pct':coverage_pct,'tp_in_budget':tp_in_budget,'alerts_in_budget':n_alerts})
    for p in percent_budgets:
        k = max(1, int(math.floor(p * len(ranked_df))))
        head = ranked_df.head(k)
        n_alerts = len(head)
        tp_in_budget = int(head['is_malicious'].sum())
        precision_pct = (tp_in_budget / n_alerts * 100.0) if n_alerts > 0 else 0.0
        coverage_pct = (tp_in_budget / total_tp * 100.0) if total_tp > 0 else float('nan')
        rows.append({'budget_type':'top_percent','budget':p,'top_n':k,'precision_pct':precision_pct,'coverage_pct':coverage_pct,'tp_in_budget':tp_in_budget,'alerts_in_budget':n_alerts})
...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("Hybrid")
    hybrid_alpha = st.slider("Hybrid alpha (anomaly + alpha * rule_z)", 0.0, 3.0, 1.0, step=0.05)
...
# ----------------------------- Hybrid rerank -----------------------------
anomalies_model = test_df[test_df['is_model_anomaly'] == 1].copy()
anomalies_model = anomalies_model.sort_values('anomaly_score', ascending=False).reset_index(drop=True)

anomalies_hybrid = anomalies_model.copy()
anomalies_hybrid['hybrid_score'] = anomalies_hybrid['anomaly_score'] + hybrid_alpha * anomalies_hybrid['rule_z']
anomalies_hybrid = anomalies_hybrid.sort_values('hybrid_score', ascending=False).reset_index(drop=True)

# ----------------------------- Budgets parsing -----------------------------
try:
    budgets_counts = [int(x.strip()) for x in budgets_str.split(',') if x.strip()]
except Exception:
    budgets_counts = [5,10,20,50,100]
try:
    percent_budgets = [float(x.strip()) for x in percent_budgets_str.split(',') if x.strip()]
except Exception:
    percent_budgets = [0.01,0.02,0.05,0.10,0.15]

total_tp = int(test_df['is_malicious'].sum())
model_table = compute_budgets_table(anomalies_model, budgets_counts, percent_budgets, total_tp)
hybrid_table = compute_budgets_table(anomalies_hybrid, budgets_counts, percent_budgets, total_tp)
...
model_tp_in_alerts = int(anomalies_model['is_malicious'].sum()) if len(anomalies_model)>0 else 0
...
<div class="kpi-row">
  ...
  <div class="kpi-card card-blue">
    <div class="kpi-title">Unique users (train/test)</div>
    <div class="kpi-value">{train_df['user'].nunique()}/{test_df['user'].nunique()}</div>
  </div>
...
<div class="kpi-row">
  <div class="kpi-card card-red">
    ...
    <div class="kpi-value">{len(anomalies_model)}</div>
  </div>
  <div class="kpi-card card-amber">
    <div class="kpi-title">Re-ranked (hybrid) alerts</div>
    <div class="kpi-value">{len(anomalies_hybrid)}</div>
  </div>
  <div class="kpi-card card-green">
...
if len(anomalies_hybrid) == 0:
    st.write("No model-detected anomalies found at the selected threshold.")
else:
    anomalies_hybrid['triggered_rules'] = anomalies_hybrid.apply(lambda r: list_triggered_rules(r, RULE_FLAGS), axis=1)
    top10_rows = anomalies_hybrid.head(10)[['user','day','hybrid_score','anomaly_score','rule_score','triggered_rules','is_malicious']]
    st.markdown('<div class="table-container monofont">', unsafe_allow_html=True)
    st.dataframe(top10_rows.reset_index(drop=True).style.set_properties(**{"font-family":"monospace"}))
...
def compute_percentile_table(ranked_df, percent_fractions, total_tp):
    ...
    for p, frac in zip(percentiles, percent_fractions):
        ...
        rows.append({'percentile_pct': p, 'top_n': k, 'alerts_in_budget': n_alerts,
                     'tp_in_budget': tp_in_budget, 'tp_pct_in_alert': tp_pct_in_alert, 'tp_pct_of_total': tp_pct_of_total})
    return pd.DataFrame(rows)

model_percent_table = compute_percentile_table(anomalies_model, percent_fractions, total_tp)
hybrid_percent_table = compute_percentile_table(anomalies_hybrid, percent_fractions, total_tp)

col_left, col_right = st.columns(2)
with col_left:
    st.subheader("Model-only TP% at percentiles")
    st.dataframe(model_percent_table.style.format({"percentile_pct":"{:.0f}", "tp_pct_of_total":"{:.2f}", "tp_pct_in_alert":"{:.2f}"}))
...
with col_right:
    ...
    st.dataframe(hybrid_percent_table.style.format({"percentile_pct":"{:.0f}", "tp_pct_of_total":"{:.2f}", "tp_pct_in_alert":"{:.2f}"}))
